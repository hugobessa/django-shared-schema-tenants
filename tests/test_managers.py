...
class SingleTenantModelManagerTests(TestCase):
    ...
    def test_create(self):
        ...
        article = Article.objects.create(title="Test Article", text="Test Article Description", author=user)

        self.assertEqual(article.tenant, self.tenant_1)

    def test_list(self):
        self.assertCountEqual(Article.objects.all(), self.tenant_1.article_set.all())
        set_current_tenant(self.tenant_2)
        self.assertCountEqual(Article.objects.all(), self.tenant_2.article_set.all())

    def test_list_passing_tenant_to_get_queryset(self):
        self.assertEqual(
            Article.objects.get_queryset(tenant=self.tenant_1).all().count(),
            self.tenant_1.article_set.all().count())
        self.assertEqual(
            Article.objects.get_queryset(tenant=self.tenant_2).all().count(),
            self.tenant_2.article_set.all().count())

    def test_list_original_queryset(self):
        self.assertEqual(Article.objects.get_original_queryset().all().count(), 8)
